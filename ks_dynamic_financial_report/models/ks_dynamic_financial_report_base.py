...
class ks_dynamic_financial_base(models.Model):
    ...
    def _prepare_account_domain(self, account_type):
        domain = [('account_type', '=', account_type.ks_account_type)]

        if account_type.main_head:
            domain.append(('main_head', '=', account_type.main_head))

        # MAIN_HEAD_FIELDS
        if account_type.assets_main_head:
            domain.append(('assets_main_head', '=', account_type.assets_main_head))
        if account_type.liability_main_head:
            domain.append(('liability_main_head', '=', account_type.liability_main_head))

        # CATEGORY_FIELDS
        if account_type.current_assets_category:
            domain.append(('current_assets_category', '=', account_type.current_assets_category))
        if account_type.fixed_assets_category:
            domain.append(('fixed_assets_category', '=', account_type.fixed_assets_category))
        if account_type.other_assets_category:
            domain.append(('other_assets_category', '=', account_type.other_assets_category))
        if account_type.current_liability_category:
            domain.append(('current_liability_category', '=', account_type.current_liability_category))
        if account_type.liability_non_current_category:
            domain.append(('liability_non_current_category', '=', account_type.liability_non_current_category))
        if account_type.equity_category:
            domain.append(('equity_category', '=', account_type.equity_category))
        if account_type.revenue_category:
            domain.append(('revenue_category', '=', account_type.revenue_category))
        if account_type.expense_category:
            domain.append(('expense_category', '=', account_type.expense_category))

        # CURRENT_ASSET_FIELDS
        if account_type.cash_equivalents_subcategory:
            domain.append(('cash_equivalents_subcategory', '=', account_type.cash_equivalents_subcategory))
        if account_type.banks_subcategory:
            domain.append(('banks_subcategory', '=', account_type.banks_subcategory))
        if account_type.accounts_receivable_subcategory:
            domain.append(('accounts_receivable_subcategory', '=', account_type.accounts_receivable_subcategory))
        if account_type.inventory_subcategory:
            domain.append(('inventory_subcategory', '=', account_type.inventory_subcategory))
        if account_type.prepaid_expenses_subcategory:
            domain.append(('prepaid_expenses_subcategory', '=', account_type.prepaid_expenses_subcategory))

        # FIXED_ASSET_FIELDS
        if account_type.vehicles_subcategory:
            domain.append(('vehicles_subcategory', '=', account_type.vehicles_subcategory))
        if account_type.furniture_fixture_subcategory:
            domain.append(('furniture_fixture_subcategory', '=', account_type.furniture_fixture_subcategory))
        if account_type.computer_printers_subcategory:
            domain.append(('computer_printers_subcategory', '=', account_type.computer_printers_subcategory))
        if account_type.machinery_equipment_subcategory:
            domain.append(('machinery_equipment_subcategory', '=', account_type.machinery_equipment_subcategory))
        if account_type.land_buildings_subcategory:
            domain.append(('land_buildings_subcategory', '=', account_type.land_buildings_subcategory))

        # OTHER_ASSET_FIELDS
        if account_type.investment_subcategory:
            domain.append(('investment_subcategory', '=', account_type.investment_subcategory))
        if account_type.vat_receivable_subcategory:
            domain.append(('vat_receivable_subcategory', '=', account_type.vat_receivable_subcategory))
        if account_type.suspense_account_subcategory:
            domain.append(('suspense_account_subcategory', '=', account_type.suspense_account_subcategory))

        # CURRENT_LIABILITY_FIELDS
        if account_type.accounts_payable_subcategory:
            domain.append(('accounts_payable_subcategory', '=', account_type.accounts_payable_subcategory))
        if account_type.short_term_loans_subcategory:
            domain.append(('short_term_loans_subcategory', '=', account_type.short_term_loans_subcategory))
        if account_type.other_liabilities_subcategory:
            domain.append(('other_liabilities_subcategory', '=', account_type.other_liabilities_subcategory))

        # NON_CURRENT_LIABILITY_FIELDS
        if account_type.long_term_loans_subcategory:
            domain.append(('long_term_loans_subcategory', '=', account_type.long_term_loans_subcategory))
        if account_type.lease_obligations_subcategory:
            domain.append(('lease_obligations_subcategory', '=', account_type.lease_obligations_subcategory))

        # EQUITY_FIELDS
        if account_type.capital_subcategory:
            domain.append(('capital_subcategory', '=', account_type.capital_subcategory))
        return domain
            ...
            if ks_report.ks_df_report_account_type == 'accounts':
                ...
                if self.ks_df_report_account_report_ids != self.env.ref(
                    ...
                    for account_type in ks_report.ks_dfr_account_type_ids:
                        domain = self._prepare_account_domain(account_type=account_type)
                        # ks_acc_id = self.env['account.account'].sudo().search([('account_type', '=', account_type.ks_account_type)])
                        ks_acc_id = self.env['account.account'].sudo().search(domain)
                        if ks_acc_id:
                    ...
                    for ks_value in ks_res[ks_report.id]['account'].values():
                        for field in ks_fields:
                            # Will Convert Equity To Positive
                            if ks_report.report_name in ["EQUITY", "Capital", "Petroraq"]:
                                ks_res[ks_report.id][field] += -1 * ks_value.get(field)
                            else:
                                ks_res[ks_report.id][field] += ks_value.get(field)
                else:
                    ks_accounts = []
    ...
    def ks_fetch_report_account_lines(self, ks_df_informations,offset={}):
        ...
        names = ks_child_reports.mapped("display_name")
    ...
    def ks_df_where_clause(self, ks_df_informations):
        ...
        # Custom Code
        company_ids_list = ks_df_informations.get('company_ids')
        if len(company_ids_list) == 1:
            custom_company_id = self.env["res.company"].sudo().browse(company_ids_list[0])
            if custom_company_id and custom_company_id.parent_id:
                company_ids_list.append(custom_company_id.parent_id.id)

        # ks_df_account_company_domain = [('company_id', 'in', ks_df_informations.get('company_ids'))]
        ks_df_account_company_domain = [('company_id', 'in', company_ids_list)]
    ...
    def ks_fetch_journal_filters(self):
        return self.env['account.journal'].search([('company_id', 'in', [self.env.company.id, self.env.company.parent_id.id])],
                                                  order="company_id, name")

    # Get account filters from model account.account
    @api.model
    def ks_fetch_account_filters(self):
        return self.env['account.account'].search([('company_id', 'in', [self.env.company.id, self.env.company.parent_id.id])],
                                                  order="company_id, name")
