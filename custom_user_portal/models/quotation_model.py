...
class PurchaseQuotation(models.Model):
    ...
    #total
    total_excl_vat = fields.Float(string="Total Amount", compute="_compute_totals", store=True)
    vat_amount = fields.Float(string="VAT Amount @ 15%", compute="_compute_totals", store=True)
    total_incl_vat = fields.Float(string="Total Amount Including VAT", compute="_compute_totals", store=True)
    is_best = fields.Boolean(string='Best Quotation', compute='_compute_is_best', store=True)
    is_best_badge = fields.Char(string='Best Quotation', compute='_compute_is_best_badge', store=False)
    ...
    budget_left = fields.Float(string='Budget Left', related='project_id.budget_left', store=False)

    # Lines
    line_ids = fields.One2many('purchase.quotation.line', 'quotation_id', string="Quotation Lines")

    @api.depends('line_ids.unit_price', 'line_ids.quantity')
    def _compute_totals(self):
        for record in self:
            total_excl = sum(line.unit_price * line.quantity for line in record.line_ids)
    ...
    def action_create_purchase_order(self):
        ...
        if self.budget_left < self.total_incl_vat:
            raise UserError("Insufficient budget. You cannot proceed with Purchase Order creation.")

        # Get 15% VAT tax
        vat_tax = self.env['account.tax'].search([('amount', '=', 15), ('type_tax_use', '=', 'purchase')], limit=1)
        ...
        purchase_order = self.env['purchase.order'].create({
            ...
            'order_line': [
                (0, 0, {
                    ...
                    'date_planned': self.expected_arrival or fields.Datetime.now(),
...
# Define group references
        procurement_admin_group = self.env.ref('custom_user_portal.procurement_admin', raise_if_not_found=False)
        project_engineer_group = self.env.ref('custom_user_portal.project_engineer', raise_if_not_found=False)
        project_manager_group = self.env.ref('custom_user_portal.project_manager', raise_if_not_found=False)
        operations_director_group = self.env.ref('custom_user_portal.operations_director', raise_if_not_found=False)
...
class PurchaseQuotationLine(models.Model):
    ...
    display_name_ui = fields.Char(
        string="Product",
        compute="_compute_display_name_ui",
        store=False
    )
    ...
    def _compute_display_name_ui(self):
        for line in self:
            line.display_name_ui = line.product_id.name if line.product_id else line.name
        ...
        def write(self, vals):
            for record in self:
                ...
                if status in ['approved', 'rejected']:
                    ...
                    _logger.info("Origin during status change: %s", record.origin or 'No origin set')

                    if record.origin:
                        prev_po = self.search([('name', '=', record.origin)], limit=1)
                        if prev_po:
                            pr_origin = prev_po.origin
                            _logger.info("Origin of origin (from %s): %s", record.origin, pr_origin or 'No origin set')

                            if pr_origin:
                                pr = self.env['purchase.requisition'].search([('name', '=', pr_origin)], limit=1)
                                if pr:
                                    supervisor_partner_id = pr.supervisor_partner_id
                                    try:
                                        supervisor_id_int = int(supervisor_partner_id)
                                        _logger.info("Supervisor partner ID (int): %s", supervisor_id_int)
                                        ...
                                        self.env['mail.activity'].create({
                                            ...
                                            'user_id': self.env['res.partner'].browse(supervisor_id_int).user_ids[:1].id,
                                        })
                                        ...
                                        mail_values = {
                                            ...
                                            'body_html': f"""
                                                ...
                                                <p>Thank you.</p>
                                            """,
                                            'email_to': self.env['res.partner'].browse(supervisor_id_int).email,
                                            'email_from': self.env.user.email or 'noreply@yourcompany.com',
                                        }
                                        self.env['mail.mail'].sudo().create(mail_values).send()

                                    except ValueError:
                                        _logger.warning("Invalid supervisor_partner_id: %s (not an integer)", supervisor_partner_id)
                                else:
                                    _logger.warning("No PR found with name: %s", pr_origin)
                        else:
                            _logger.info("No previous PO found with name: %s", record.origin)
