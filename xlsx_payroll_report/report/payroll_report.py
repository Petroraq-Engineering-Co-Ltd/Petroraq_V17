...
class PayrollReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        for used_struct in used_structures:
            ...
            for sa_rule in salary_rule_ids:
                if sa_rule.name not in order:
                    order.append(sa_rule.name)

            # Create a dictionary to map name to its position in the order
            order_map = {name: index for index, name in enumerate(order)}

            # Sort the rules list based on the custom order
            sorted_rules = sorted(salary_rule_ids, key=lambda x: order_map[x.name])
            ...
            sheet.write(2,0,'Employee ID',format1)
            ...
            for slip in lines.slip_ids:
                if lines.slip_ids:
                    if slip.struct_id.id == used_struct[0]:
                        has_payslips = True
                        sheet.write(e_name, 0, slip.employee_id.code, format3)
